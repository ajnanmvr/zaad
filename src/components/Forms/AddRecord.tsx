...
const AddRecord = ({ type, edit }: { type: string; edit?: boolean }) => {
  ...
  const [searchSuggestions, setSearchSuggestions] = useState<Array<TBaseData & { entityType: "company" | "employee" | "individual" }>>([]);
  ...
  const [paymentStatusOptions, setPaymentStatusOptions] = useState<Array<{ value: string; label: string; color?: string; appliesTo?: "income" | "expense" | "both" }>>([]);
  ...
  const fetchBalance = useCallback(async (Id?: string, entityType?: string) => {
    try {
  ...
  }, [fetchBalance, isLockedEntity, lockEntityType, lockEntityId, lockEntityName]);
  ...
  const fetchsearchSuggestions = async (inputValue: string) => {
    try {
      ...
      const companies = (companiesRes.data || []).map((item) => ({ ...item, entityType: "company" as const }));
      const employees = (employeesRes.data || []).map((item) => ({ ...item, entityType: "employee" as const }));
      const individuals = (individualsRes.data || []).map((item) => ({ ...item, entityType: "individual" as const }));
  ...
  const fetchPrev = useCallback(async (Id?: string) => {
    try {
      if (!edit) {
  ...
  const handleEntitySelection = (selected: TBaseData & { entityType: "company" | "employee" | "individual" }) => {
    if (!selected._id) {
      return;
    }
    ...
    setRecordData((prev) => ({
      ...
      employee: targetType === "employee" || targetType === "individual" ? selected._id : undefined,
      self: undefined,
    }));
    setSearchSuggestions([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([paymentRes, statusRes]) => {
        const methodOptions = (paymentRes.data?.options || []).map((item: any) => ({
          value: item.method,
  ...
  const applyOfficeExpensePreset = () => {
    ...
    const officeStatus = filteredStatusOptions.find((item) => item.value.toLowerCase().includes("office"));
  ...
  const applyLiabilityPreset = () => {
    const liabilityStatus = filteredStatusOptions.find((item) => item.value.toLowerCase().includes("liability"));
