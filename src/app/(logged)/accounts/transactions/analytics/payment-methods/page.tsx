...
export default function AnalyticsPaymentMethodsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100">Latest Month Payment Methods</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Full payment method balances for {latest ? monthLabel(latest.month, latest.year) : "the latest cached month"}, including carry-forward balances.
          </p>
        </div>
        <Link
            ...
            <table className="w-full min-w-[860px]">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="px-3 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">Method</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {rows.length > 0 ? rows.map((item, index) => (
                  <tr key={item.methodId} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="px-3 py-3 text-sm font-semibold text-slate-900 dark:text-slate-100">#{index + 1} {item.methodLabel}</td>
                  ...
                  <tr>
                    <td colSpan={4} className="px-3 py-10 text-center text-sm text-slate-500 dark:text-slate-400">No payment method data found in latest month.</td>
                  </tr>
                )}
              </tbody>
            </table>
