...
import { getCompanyAction, deleteCompanyAction, deleteCompanyDocumentAction, updateCompanyDocumentAction } from "@/actions/company-employee";
...
const SingleCompany = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await getCompanyAction(id)
  ...
  const confirmDelete = async () => {
    ...
    await deleteCompanyDocumentAction(id, selectedDocumentId!);
    fetchData();
  ...
  const saveEdits = async () => {
    ...
    await updateCompanyDocumentAction(id, selectedDocumentId!, editData);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="rounded-lg bg-white p-8 shadow-lg dark:bg-black">
            ...
            <div className="flex justify-end">
              <button onClick={closeModal} className="mr-2 rounded-lg bg-gray-300 px-4 py-2 hover:bg-gray-400">
                Cancel
              </button>
              <button onClick={saveEdits} className="rounded-lg bg-primary px-4 py-2 text-white hover:bg-red-600">
                Update
              </button>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-default dark:bg-boxdark">
          <div className="px-6 py-8 sm:p-10">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-3xl font-bold capitalize text-black dark:text-white">{company?.name}</h2>
                <p className="mb-8 capitalize text-gray-600 dark:text-gray-400">{company?.companyType}</p>
              </div>
              <div className="flex items-center gap-1">
                <div className="relative">
                  <Link
                    ...
                    href="#"
                  >
                    <span className="hidden text-right capitalize lg:block">
                  ...
                  <div
                    ...
                    className={`absolute right-0 mt-4 flex w-40 flex-col rounded-sm border border-stroke bg-white py-3 shadow-default dark:border-strokedark dark:bg-boxdark ${dropdownOpen === true ? "block" : "hidden"}`}
                  >
                    {user?.role === "partner" && (
                      <>
                        <Link
                          href={`/accounts/transactions/company/${id}`}
                          className="px-6 py-2 text-sm font-medium duration-300 ease-in-out hover:bg-primary hover:bg-opacity-10 hover:text-primary lg:text-base"
                        >
                          Records
                        </Link>
                      </>
                    ...
                    <button
                      ...
                      className="flex justify-start px-6 py-2 text-sm font-medium text-red duration-300 ease-in-out hover:bg-red hover:bg-opacity-10 lg:text-base"
                    >
                      Delete
                    </button>
                  </div>
            ...
            <div className="grid min-h-44 grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                <h3 className="mb-2 text-xl font-semibold text-black dark:text-white">Company Details</h3>
                <ul className="flex flex-col">
                  {company?.licenseNo && (
                    <li>
                      License No:
                      <span className="mx-1 break-words font-medium">{company.licenseNo}</span>
                    </li>
                  )}
                  {company?.emirates && (
                    <li>
                      Emirates/Area:
                      <span className="mx-1 break-words font-medium">{company.emirates}</span>
                    </li>
                  )}
                  {company?.phone1 && (
                    <li>
                      Phone 1:
                      <span className="mx-1 break-words font-medium">{company.phone1}</span>
                    </li>
                  )}
                  {company?.phone2 && (
                    <li>
                      Phone 2:
                      <span className="mx-1 break-words font-medium">{company.phone2}</span>
                    </li>
                  )}
                  {company?.email && (
                    <li>
                      Email:
                      <span className="mx-1 break-words font-medium">{company.email}</span>
                    </li>
                  )}
                  {company?.transactionNo && (
                    <li>
                      Transaction No:
                      <span className="mx-1 break-words font-medium">{company.transactionNo}</span>
                    </li>
                  )}
                  {company?.isMainland && (
                    <li>
                      Mainland/Freezone:
                      <span className="mx-1 break-words font-medium">{company.isMainland}</span>
                    </li>
                  )}
                  {company?.remarks && (
                    <li>
                      Remarks:
                      <span className="mx-1 break-words font-medium">{company.remarks}</span>
                    </li>
                  )}
                </ul>
              </div>
              ...
              {company?.password?.length !== 0 && (
                <div className="">
                  <h3 className="mb-2 text-xl font-semibold text-black dark:text-white">Passwords.</h3>
                  <ul>
                    {company?.password?.map((pass, index) => (
                      <li key={index}>
                        <span className="font-medium">{pass.platform}:</span>
                        <span className="mx-1 break-words font-medium">{pass.username}</span>
                        <span className="mx-1 break-words font-medium">{pass.password}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <table className="w-full table-auto">
                    <thead>
                      <tr className="bg-gray-2 text-left dark:bg-meta-4">
                        <th className="min-w-[220px] px-4 py-4 font-medium text-black dark:text-white xl:pl-11">Name</th>
                        <th className="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">Issue Date</th>
                        <th className="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">Expiry Date</th>
                      ...
                      {company?.documents?.map(({ name, status, issueDate, expiryDate, _id }, key) => (
                        <tr key={key}>
                          <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                            <h5 className="font-medium capitalize text-black dark:text-white">{name || "---"}</h5>
                          </td>
                          <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p className="text-black dark:text-white">{issueDate || "---"}</p>
                          </td>
                          <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p className="text-black dark:text-white">{expiryDate || "---"}</p>
                          </td>
                          <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p
                          ...
                          <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="rounded p-1 hover:bg-primary hover:bg-opacity-10"
                              >
